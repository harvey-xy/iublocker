...
`isUrlFilterCaseSensitive` is `false` unless `$match-case`, for `regexFilter` as well as
`urlFilter`.

**The `requestDomains` rewrite needs a real hostname.** `requestDomains` matches a domain
_or one of its subdomains_, which is the `||host^` meaning only when the anchored text is
actually a hostname. Lists are full of `||` patterns that are host **prefixes** —
`||adservice.google.` (every Google ad ccTLD), `||ad120m.`, `||e-zpass.com-` (phishing
hosts), `||142.91.159.` — and as `requestDomains` entries those match nothing whatsoever.
The compiler therefore only rewrites a `||host` / `||host^` pattern when `host` has a dot
and is a valid hostname; everything else keeps the `urlFilter`, where DNR's own `||` anchor
has the ABP meaning. `$to=`/`$denyallow=` are unaffected: `$to=com` really does mean "every
`.com` host". A pattern that is nothing but anchors (`@@||$domain=…`) is dropped — it used
to compile to `urlFilter: "||"`, an unbounded allow.
...
| `$removeparam=name`                                                                                                                                                                                                             | `action.redirect.transform.queryTransform.removeParams: ["name"]`                                                                                          | ✅ Only exact names; `~` and regex dropped, empty (`$removeparam` alone) only valid on `@@` |
...
| `$header=name[:value]`                                                                                                                                                                                                          | `condition.responseHeaders: [{header, values?}]`; `~` → `excludedResponseHeaders`                                                                          | ✅ Chrome ≥ 128; the value is wrapped in `*` (see below), a `/regex/` value is dropped      |
...
**`$header=` values.** uBO matches the value _unanchored_ (`$header=via:1.1 google` matches
`Via: 1.1 google (GFE)`), while DNR's `responseHeaders.values` are `*`/`?` globs matched
against the whole header value. The compiler wraps the value in `*` so the two agree; a
`/regex/` value cannot be expressed at all and drops the filter with that reason rather
than shipping a rule that silently never matches (which would also turn an `@@` exception
into a no-op).

**Exceptions carrying a modifier** take the resource types of the rules they cancel, not the
`@@` default: `@@…$csp` / `@@…$permissions` become main_frame + sub_frame (the types `$csp`
rules use) and `@@…$removeparam` covers every type, because otherwise the exception could
never cancel the modifier on a navigation — which is exactly what those filters exist for.
`@@…$redirect` and `@@…$redirect-rule` with no value are valid exceptions (uBO: "do not
redirect this"), not parse errors, and compile to a plain `allow`.

Unknown options make the whole filter invalid (dropped with a warning), matching uBO.
`$top=` and `$requestheader=` are named as unsupported instead: DNR has no top-level-document
domain condition and no request-header condition.
...
| 5    | `@@…$document` (allowAllRequests) from lists; `$important` raises it to `max(3, 5)` rather than lowering it to 3                                               | 5          |
