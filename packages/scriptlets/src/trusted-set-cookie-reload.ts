...
export default defineScriptlet({
  ...
  args: [
    ...
    { name: 'value', doc: 'Any value; `$now$` / `$currentDate$` expand, `$remove$` deletes it.' },
    { name: 'offsetExpiresSec', optional: true, doc: 'Lifetime in seconds, or "1year" / "1day".' },
    { name: 'path', optional: true, doc: 'Cookie path, default "/".' },
    { name: 'extra1', optional: true, doc: 'Trailing `name, value` extra argument (e.g. `domain`).' },
    ...
    try {
      ...
      const already = String(doc.cookie ?? '')
        ...
        .some(function (part: string) {
          return part.trim().startsWith(encoded + '=');
        });
      ...
      if (p === 'none') p = '';
      let cookie = encoded + '=' + encodeURIComponent(remove ? '' : out);
      if (p !== '') cookie += '; path=' + p;
      ...
      doc.cookie = cookie;
      if (already) return;
      const flag = '__iub_scr_' + encoded;
      try {
        const store: any = gt.sessionStorage;
        if (store !== undefined && store !== null) {
          if (store.getItem(flag) === '1') return;
          store.setItem(flag, '1');
        } else if (gt[flag] === true) {
          return;
        } else {
          gt[flag] = true;
        }
