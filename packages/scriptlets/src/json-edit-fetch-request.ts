  ...
  fn: function (path: string, ...extra: string[]) {
    try {
      ...
      gt.fetch = keep(origFetch, function (this: any, input?: any, init?: any, ...rest: any[]): any {
        const details: Record<string, any> = { method: 'GET' };
        let nextInit = init;
        try {
          if (typeof input === 'string') details['url'] = input;
          ...
          if (matches(details) && init !== null && typeof init === 'object' && typeof init.body === 'string') {
            const obj = JSON.parse(init.body);
            if (edit(obj)) nextInit = Object.assign({}, init, { body: JSON.stringify(obj) });
          }
        } catch {
          /* not a JSON body */
        }
        return origFetch.call(this === undefined ? gt : this, input, nextInit, ...rest);
      });
    } catch {
