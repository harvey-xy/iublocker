...
export default defineScriptlet({
  name: 'trusted-edit-inbound-object',
  args: [
  ...
  fn: function (propChain: string, argpos: string, path: string) {
    try {
      ...
      const index = parseInt(String(argpos ?? '0'), 10);
      if (isNaN(index) || index < 0) return;
      const mark = Symbol.for('iub.editInbound.' + propChain + '#' + String(index));
      if (owner[mark] === true) return;
      const patched = keep(orig, function (this: any, ...args: any[]): any {
        try {
          const subject = args[index];
          if (subject !== null && typeof subject === 'object') edit(subject);
        } catch {
