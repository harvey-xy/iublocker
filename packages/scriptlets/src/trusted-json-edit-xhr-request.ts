...
export default defineScriptlet({
  ...
  fn: function (path: string, ...extra: string[]) {
    try {
      ...
      XHR.prototype.send = keep(origSend, function (this: any, body?: any, ...rest: any[]): any {
        let next = body;
        try {
          const ctx = this[ctxKey];
          if (ctx !== undefined && matches(ctx) && typeof body === 'string') {
            const obj = JSON.parse(body);
            if (edit(obj)) next = JSON.stringify(obj);
          }
        } catch {
          /* not a JSON body */
        }
        return origSend.call(this, next, ...rest);
      });
    } catch {
