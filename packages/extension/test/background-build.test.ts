...
import { buildManifest, buildRuleResources, parseArgs, runBuild, type ChromeManifest } from '../scripts/build';
...
describe('build: rule_resources', () => {
  ...
  it('only enables an already shipped e2e ruleset', () => {
    const withE2e = makeRulesetManifest({ lists: [makeListEntry('e2e-test', { defaultEnabled: false, group: 'test' })] });
...
describe('build: manifest generation', () => {
  it('drops content scripts whose bundle was not built', () => {
    const warnings: string[] = [];
    const manifest = buildManifest('9.9.9', [], () => false, (m) => warnings.push(m));
  ...
  it('keeps them when the file exists', () => {
    const manifest = buildManifest('1.0.0', [], () => true, () => undefined);
...
describe('build: smoke test', () => {
  it(
    'builds a loadable dist/ and never throws on missing entry points',
    async () => {
      ...
      const info = JSON.parse(await readFile(path.join(outDir, 'build-info.json'), 'utf8')) as Record<string, unknown>;
  ...
  it(
    'fails fast with --strict when the compiler output is missing',
    async () => {
      const outDir = await tempDir();
